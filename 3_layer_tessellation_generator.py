...
class VariableGeometryGenerator:
    def __init__(self, filename, cols, rows, cell_size, min_margin, max_margin, min_tab_width, max_tab_width, bridge_size):
        ...
        self.green_stroke_style = {'stroke': 'green', 'stroke_width': 0.1, 'fill': 'none'}
            ...
            if has_mid_bridge:
                ...
                if t_start < t_mid_left:
                    dwg.add(dwg.line(start=(p1[0] + ux * t_start, p1[1] + uy * t_start),
                                     end=(p1[0] + ux * t_mid_left, p1[1] + uy * t_mid_left), **style))
                if t_mid_right < t_end:
                    dwg.add(dwg.line(start=(p1[0] + ux * t_mid_right, p1[1] + uy * t_mid_right),
                                     end=(p1[0] + ux * t_end, p1[1] + uy * t_end), **style))
            else:
                dwg.add(dwg.line(start=(p1[0] + ux * t_start, p1[1] + uy * t_start),
                                 end=(p1[0] + ux * t_end, p1[1] + uy * t_end), **style))
...
class VariableTabbedGrid(VariableGeometryGenerator):
    def generate(self):
        ...
        self._draw_grid_layer(dwg, is_inverted=False, style=self.stroke_style)
        self._draw_grid_layer(dwg, is_inverted=True, style=self.green_stroke_style)

        self._draw_boundary_gaps(dwg, is_inverted=False, style=self.stroke_style)
        self._draw_boundary_gaps(dwg, is_inverted=True, style=self.green_stroke_style)

        self._draw_red_layer(dwg)
        self.save_drawing(dwg)

    def _draw_grid_layer(self, dwg, is_inverted, style):
        for row in range(self.rows):
            for col in range(self.cols):
                ...
                if row % 2 == 0 and col % 2 == 0:
                    ...
                    if is_large_square:
                        self.draw_square(dwg, x, y, self.cell_size, True, row, col, style)
                    else:
                        size = self.cell_size - (2 * m)
                        self.draw_square(dwg, x + m, y + m, size, False, row, col, style)
                ...
                # 2. Horizontal Rectangles (Even Row, Odd Col)
                    ...
                    if is_inverted:
                        left_is_large = not left_is_large

                    if left_is_large:
                        end_x = x + self.cell_size - m_right
                        self.draw_solid_line(dwg, (x, y1_L), (end_x, y1_R), style)     # Top
                        self.draw_solid_line(dwg, (end_x, y1_R), (end_x, y2_R), style) # Right
                        self.draw_solid_line(dwg, (end_x, y2_R), (x, y2_L), style)     # Bottom
                        # Skipped Left side because it perfectly overlaps the bridged Large Square
                    else:
                        start_x = x + m_left
                        end_x = x + self.cell_size
                        self.draw_solid_line(dwg, (start_x, y1_L), (end_x, y1_R), style) # Top
                        self.draw_solid_line(dwg, (start_x, y1_L), (start_x, y2_L), style) # Left
                        self.draw_solid_line(dwg, (start_x, y2_L), (end_x, y2_R), style) # Bottom
                ...
                # 3. Vertical Rectangles (Odd Row, Even Col)
                    ...
                    if is_inverted:
                        top_is_large = not top_is_large

                    if top_is_large:
                        end_y = y + self.cell_size - m_bottom
                        self.draw_solid_line(dwg, (x1_T, y), (x1_B, end_y), style)       # Left
                        self.draw_solid_line(dwg, (x2_T, y), (x2_B, end_y), style)       # Right
                        self.draw_solid_line(dwg, (x1_B, end_y), (x2_B, end_y), style)   # Bottom
                        # Skipped Top side because it perfectly overlaps the bridged Large Square
                    else:
                        start_y = y + m_top
                        end_y = y + self.cell_size
                        self.draw_solid_line(dwg, (x1_T, start_y), (x1_B, end_y), style) # Left
                        self.draw_solid_line(dwg, (x2_T, start_y), (x2_B, end_y), style) # Right
                        self.draw_solid_line(dwg, (x1_T, start_y), (x2_T, start_y), style) # Top
                        # Skipped Bottom side because it perfectly overlaps the bridged Large Square

                # 4. Alternating X-Tab Pattern (Odd Row, Odd Col)
                elif row % 2 != 0 and col % 2 != 0:
                    self._draw_x_tabs(dwg, x, y, row, col, is_inverted, style)

    def _draw_x_tabs(self, dwg, x, y, row, col, is_inverted, style):
        ...
        dwg.add(dwg.line(start=(tl_x, tl_y - d), end=(cx, cy - d), **style))
        dwg.add(dwg.line(start=(cx + d, cy), end=(br_x + d, br_y), **style))
        dwg.add(dwg.line(start=(tl_x - d, tl_y), end=(cx - d, cy), **style))
        dwg.add(dwg.line(start=(cx, cy + d), end=(br_x, br_y + d), **style))

        # --- Two parallel lines at -45° (/) ---
        dwg.add(dwg.line(start=(bl_x - d, bl_y), end=(cx - d, cy), **style))
        dwg.add(dwg.line(start=(cx, cy - d), end=(tr_x, tr_y - d), **style))
        dwg.add(dwg.line(start=(bl_x, bl_y + d), end=(cx, cy + d), **style))
        dwg.add(dwg.line(start=(cx + d, cy), end=(tr_x + d, tr_y), **style))

    def _draw_boundary_gaps(self, dwg, is_inverted, style):
        # Top boundary (row = 0)
        row = 0
        y = 0
        for col in range(self.cols - 1):
            ...
            if col % 2 == 0: # Square at col, H-Rect at col+1
                ...
                self.draw_solid_line(dwg, TR, TL_next, style)
        ...
        for col in range(self.cols - 1):
            ...
            if col % 2 == 0: # Square at col, H-Rect at col+1
                ...
                self.draw_solid_line(dwg, BR, BL_next, style)
            else: # H-Rect at col, Square at col+1
                m_right = self.get_margin(row, col + 1)
                right_is_large = (row // 2 + (col + 1) // 2) % 2 == 0
                if is_inverted: right_is_large = not right_is_large
        ...
        for row in range(self.rows - 1):
            ...
            if row % 2 == 0: # Square at row, V-Rect at row+1
                ...
                self.draw_solid_line(dwg, BL, TL_next, style)
        ...
        for row in range(self.rows - 1):
            ...
            if row % 2 == 0: # Square at row, V-Rect at row+1
                ...
                self.draw_solid_line(dwg, BR, TR_next, style)
            else: # V-Rect at row, Square at row+1
                m_bottom = self.get_margin(row + 1, col)
                bottom_is_large = ((row + 1) // 2 + col // 2) % 2 == 0
                if is_inverted: bottom_is_large = not bottom_is_large
...
def main():
    # Best to use odd number of columns and rows for better tessellation.
    GRID_COLS = 9
    GRID_ROWS = 7
    CELL_SIZE = 15.0
    ...
    output_filename = f"SVGs/Array_{next_number}_3_layer.svg"

    print(f"Generating {output_filename} with triple layer support...")
