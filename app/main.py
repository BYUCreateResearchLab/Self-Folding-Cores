...
if enable_curvature:
    ...
    normal_gap_x = st.sidebar.slider("Bottom Layer Vertical Gap (mm)", 0.0, float(max_w), 3.0, 0.1, key="normal_gap_x")
    normal_gap_y = st.sidebar.slider("Bottom Layer Horizontal Gap (mm)", 0.0, float(max_h), 3.0, 0.1, key="normal_gap_y")
    alt_gap_x = st.sidebar.slider("Top Layer Vertical Gap (mm)", 0.0, float(max_w), 1.0, 0.1, key="alt_gap_x")
    alt_gap_y = st.sidebar.slider("Top Layer Horizontal Gap (mm)", 0.0, float(max_h), 1.0, 0.1, key="alt_gap_y")

    st.sidebar.subheader("S-Curve Gap Transition")
    s_curve_axis = st.sidebar.selectbox("Transition Axis", ["X", "Y"], key="s_curve_axis")
    max_point = int(cols) if s_curve_axis == "X" else int(rows)
    label_point = "Transition Column" if s_curve_axis == "X" else "Transition Row"
    s_curve_point = st.sidebar.number_input(label_point, min_value=0, max_value=max_point, value=max_point // 2, key="s_curve_point")
    s_curve_cells = st.sidebar.number_input("Transition Cells (Width of S-Curve)", min_value=0, max_value=max_point, value=min(3, max_point), key="s_curve_cells")
    max_t_gap = max_w if s_curve_axis == "X" else max_h
    s_curve_transition_gap = st.sidebar.slider("Transition Gap (mm)", 0.0, float(max_t_gap), 2.0, 0.1, key="s_curve_transition_gap")
else:
